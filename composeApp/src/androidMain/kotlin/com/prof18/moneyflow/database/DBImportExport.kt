...
actual class DBImportExport(
    ...
    actual fun generateDatabaseFile(): Any? {
        ...
        try {
            ...
            openFileOutput.use { output ->
                ...
                return context.getFileStreamPath(DatabaseHelper.DB_FILE_NAME_WITH_EXTENSION)
            }
        } catch (e: Exception) {
    ...
    actual fun exportDatabaseToFileSystem(uri: Any): MoneyFlowResult<Unit> {
        ...
        try {
            ...
            val folder = File(destPath + File.pathSeparator + DatabaseHelper.DB_FILE_NAME)
            if (!folder.exists()) {
                folder.mkdirs()
            }

            val openOutputStream = context.contentResolver.openOutputStream(parsedUri)
    ...
    actual fun importDatabaseFromFileSystem(uri: Any): MoneyFlowResult<Unit> {
        ...
        try {
            val openInputStream = context.contentResolver.openInputStream(parsedUri)
    ...
    actual fun databasePath(): String = context.getDatabasePath(DatabaseHelper.DATABASE_NAME).toString()

    private fun requireUri(uri: Any): Uri {
        return uri as? Uri ?: error("Expected android.net.Uri")
    }
